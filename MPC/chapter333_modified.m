...
Nu = 2; % 控制量共有2个：车辆的纵向速度（后轴速度），前轮偏角
...
%% 定义相关矩阵
% x_real矩阵，用于存储每一个仿真时刻，车辆的实际位置状态，初始状态为上述定义的X0
...
R = 0.1*eye(Nu*Tsim,Nu*Tsim); % 最终的状态（控制量）加权矩阵，用于调节“控制量偏差”和“状态量偏差”在目标函数中的比重
...
for i = 1:1:Nr
    ...
    u_piao(i,1) = X(1,1); % 取最优解序列的第一个元素,纵向速度
    u_piao(i,2) = X(2,1); % 取最优解序列的第一个元素,前轮转角
